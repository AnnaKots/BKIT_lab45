    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (fd.ShowDialog()==DialogResult.OK)
            {
                ...
                string text = File.ReadAllText(fd.FileName);//метод чтения файла в виде строки
                
                char[] separators = new char[] { ' ', '.', ',', '!', '?', '/', '\t', '\n' };//Разделительные символы для чтения из файла
                string[] textArray = text.Split(separators);//Делим на слова
                foreach (string strTemp in textArray)
                {
                    
                    string str = strTemp.Trim();//Удаление пробелов в начале и конце строки

                    if (!list.Contains(str))// проверка
                        list.Add(str);//Добавление строки в список, если строка не содержится в списке
                }
                t.Stop();
